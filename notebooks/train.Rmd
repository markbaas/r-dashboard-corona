...
```{r}
data <- read_rds("../results/{params$data}.Rds" %>% glue())
head(data)
```
...
cols <- c("Temperature", "PopElder", "PM2.5", "PM10", "NO2", "O3", "RegioS", mcols)
# cols <- c("Temperature", "PM2.5", mcols)
pcols <- c("MortalityRate.-5", "MortalityRate.-6", "MortalityRate.-7", "MortalityRate.-8", "Temperature",  "PM2.5", "NO2", "PopElder")
...
fitControl <- trainControl(method = "repeatedcv", number = 50, repeats = 20, search = "random", allowParallel = F, verboseIter = F)
fitControl2 <- trainControl(method = "repeatedcv", number = 10, repeats = 10, search = "random", allowParallel = F, verboseIter = F)

tic()
model.cv <- train(
  MortalityRate ~ ., 
  data = data_train %>% select_if(~ length(unique(.)) > 1), 
  method = "lasso", 
...
model.spatial <- train(
  MortalityRate ~ ., 
  data = data_train.spatial %>% select_if(~ length(unique(.)) > 1), 
  method = "lasso", 
